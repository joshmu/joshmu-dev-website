import { MotionValue, motion, useSpring, useTransform, useScroll } from "framer-motion";
...
const CharSplit = ({ children, scrollStart, scrollEnd, scrollYProgress }: CharSplitProps) => {
  return (
    <>
...
const Spray = ({ char, idx, scrollStart = 0, scrollEnd = 1, scrollYProgress }: SprayProps) => {
  ...
  const x = useTransform(scrollYProgress, [scrollStart, scrollEnd], [0, maxDistance.current]);
  const y = useTransform(scrollYProgress, [scrollStart, scrollEnd], [0, maxDistance.current]);
  ...
  return (
    <span ref={ref}>
      <motion.span className="absolute left-0 z-0" style={{ x: xVel, y: yVel, scale, rotate }}>
        {char}
      </motion.span>
    </span>
  );
...
// helper
const randomNum = (min: number, max: number): number => Math.floor(Math.random() * max) + min;
